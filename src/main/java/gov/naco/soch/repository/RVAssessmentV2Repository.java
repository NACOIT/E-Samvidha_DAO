...
import gov.naco.soch.entity.RVAssessmentV2;
import gov.naco.soch.projection.TiBenRVAssessmentProjection;


@Repository
public interface RVAssessmentV2Repository extends JpaRepository<RVAssessmentV2, Long>, JpaSpecificationExecutor<RVAssessmentV2> {
    ...
    Page<TiBenRVAssessmentProjection> findTiBenAssessmentByBasicSearchWithTsVector(
			@Param("facilityId") Long facilityId, 
			@Param("isDeleted") Boolean isDeleted,@Param("searchString") String searchString,
			@Param("dueDate") LocalDate dueDate, @Param("endDueDate") LocalDate endDueDate,
			Pageable pageable);
	...
	String missedAssessmentquery ="select trv.id as id,trv.due_date_of_assessment as dueDate, trv.facility_id as facilityId,\r\n" + 
			...
			"ben.last_name as lastname,ben.date_of_birth as dateOfBirth,bg.name as gender\r\n" + 
			"from soch.ti_ben_rv_assessment  trv \n" +
			" join soch.ti_beneficiary tiben on tiben.id=trv.beneficiary_id \n" +
			" join soch.beneficiary ben on ben.id=tiben.beneficiary_id \n" +
			" join soch.master_client_status_ticore msc on tiben.status_id=msc.id \n" +
			" join soch.typology_master tm on tm.typology_id = tiben.master_hrg_primary_id \n" +
			" join soch.master_gender bg on ben.gender_id = bg.id " +
			" where trv.due_date_of_assessment<=:dueDate and msc.id = 1 and trv.is_early = false \n" +
			" and trv.is_delete=:isDeleted and trv.facility_id=:facilityId and ben.ti_benf_search_str ilike :searchString \n";

	String missedAssessmentCount=" select count(*) \n" +
			" from soch.ti_ben_rv_assessment  trv \n" +
			" join soch.ti_beneficiary tiben on tiben.id=trv.beneficiary_id \n" +
			" join soch.beneficiary ben on ben.id=tiben.beneficiary_id \n" +
			" join soch.master_client_status_ticore msc on tiben.status_id=msc.id \n" +
